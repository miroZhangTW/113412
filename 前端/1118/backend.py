...
def detect_recommend():
    ...
    health = float(analysis_data.get('health', 0))
    stain = float(analysis_data.get('stain', 0))
    dark_circle = float(analysis_data.get('dark_circle', 0))
    acne = float(analysis_data.get('acne', 0))

    # 條件設定
    conditions = {
        "health": ("值超過50 不推薦", health, ["柔膚", "中性肌", "全能", "調理", "極潤", "緊緻", "輕透", "全效", "肌淨", "維他命"]),
        "stain": ("值低於50 不推薦", stain, ["無暇", "淨白", "美白", "亮白", "光澤", "撫平", "亮采", "瞬白", "透白", "潤色", "提亮"]),
        "dark_circle": ("值低於50 不推薦", dark_circle, ["無暇", "淨白", "美白", "亮白", "柔霧", "亮采", "較色", "瞬白", "透白", "潤色"]),
        "acne": ("值低於40 不推薦", acne, ["控油", "保濕", "痘痘肌", "抗痘", "敏感肌", "透白", "維他命", "輕透", "零妝感"])
    }
    ...
    try:
        ...
        for condition, (rule, value, keywords) in conditions.items():
            if (condition == "health" and value > 50) or (condition in ["stain", "dark_circle"] and value < 50) or (condition == "acne" and value < 40):
                continue  # 不符合推薦條件則跳過

            for table in product_tables:
                sql = f"""
                    SELECT Name, Brand, Category, `Size and Price`, ImageURL
                    FROM {table}
                ...
                """
                products = db.getSqlData(sql)
                for product in products:
                    # 直接顯示整個 "Size and Price" 欄位的內容
                    size_and_price = product[3]

                    recommended_products.append({
                        'name': product[0],
                        'brand': product[1],
                        'category': product[2],
                        'size_and_price': size_and_price,  # 直接使用這個欄位的值
                        'image_url': product[4]
                    })
