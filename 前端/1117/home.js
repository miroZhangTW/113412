...
function onImageUploadSuccess(data) {
    ...
    if (data.image_url && data.analysis_result) {
        // 顯示圖片
        ...
        analysisResultDiv.innerHTML = `
            <img src="${data.image_url}" alt="上傳圖片" style="max-width: 100%; border-radius: 8px; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.1);">
            <p><strong>分析結果:</strong></p>
            <p>健康度: ${data.analysis_result.health} (值越低越健康)</p>
            <p>斑點值: ${data.analysis_result.stain} (值越高，斑點越明顯)</p>
            <p>黑眼圈值: ${data.analysis_result.dark_circle} (值越高，黑眼圈越明顯)</p>
            <p>痘痘值: ${data.analysis_result.acne} (值越高，痘痘越嚴重)</p>
        `;
    } else {
        // 如果沒有分析結果，顯示錯誤訊息
        uploadStatus.textContent = data.error || "圖片上傳失敗。";
        analysisResultDiv.innerHTML = "<p>分析失敗或無法檢測到人臉。</p>";
    }
}

// 上傳圖片的函數

// 如果圖片上傳成功後，執行計算膚質類型的函數
function uploadImage() {
    ...
    if (file) {
        ...
        if (file.size > 2 * 1024 * 1024) {
            statusMessage.textContent = "圖片大小超過限制（2MB）。請選擇較小的圖片。";
            return;
        }

        const formData = new FormData();
        formData.append("file", file);  // 確保 key 名稱與後端一致
        ...
        .then(data => {
            if (data.image_url) {
                statusMessage.textContent = "圖片上傳成功！";
                ...
                statusMessage.textContent = data.error || "圖片上傳失敗。";
            }
        })
        .catch(error => {
            console.error('Error:', error);
            // 顯示詳細錯誤訊息
            statusMessage.textContent = "圖片上傳時發生錯誤：" + error.message;
        });
    } else {
        statusMessage.textContent = "請選擇圖片。";
    }
}
if (data.image_url) {
    statusMessage.textContent = "圖片上傳成功！";
