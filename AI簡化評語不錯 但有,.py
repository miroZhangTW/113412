...
def simplify_review(text):
    try:
        ...
        # 提取前幾個字（避免刪掉 "某某人" 的情況）
        first_few_words = cleaned_text[:10]  # 提取前10個字以做判斷

        # 確保輸入文本的長度
        if len(cleaned_text) < 50:
            return text  # 如果文本過短，返回原始文本
        ...
        # 如果開頭提到 "人" 並且被刪減，顯示出前幾個字的內容
        if "人" in first_few_words:
            summary = f"原文提及：'{first_few_words.strip()}' " + summary

        return summary

    except re.error as regex_err:
        error_message = f"Regex error: {regex_err}"
    except IndexError as index_err:
        error_message = f"Indexing error: {index_err}"
    except Exception as general_err:
        error_message = f"Unexpected error: {general_err}"

    # 返回錯誤信息，讓使用者知道處理失敗的原因
    return f"處理失敗，原因: {error_message}"
